(function () {
	...
	/**
	 * Brengt een specifiek UI-element naar de voorgrond (hoogste Z-index).
	 * @param {HTMLElement} element - Het element dat naar voren moet komen.
	 */
	...
	/**
	 * Sluit de secundaire vensters (chat en inventaris), maar laat het bouwmenu open.
	 */
	...
	/**
	 * Sluit alle vensters, inclusief het bouwmenu.
	 * Wordt gebruikt bij acties die een "schoon" scherm vereisen, zoals iets oppakken.
	 */
	...
	/**
	 * Berekent de tegels die direct "achter" een object liggen vanuit het perspectief van de camera.
	 * Dit wordt gebruikt voor diepte-sortering en transparantie.
	 * @param {Object} obj - Het object (bv. speler of meubel).
	 * @returns {Array} Lijst van coördinaten {x, y}.
	 */
	...
	/**
	 * Rendert de items die in de inventaris van de speler zitten naar de UI.
	 */
	...
	// Collision check
	/**
	 * Controleert of een tegel geblokkeerd is door een meubelstuk.
	 * @param {number} x - Tegel X coördinaat.
	 * @param {number} y - Tegel Y coördinaat.
	 * @returns {boolean} True als de tegel bezet is.
	 */
	...
	/**
	 * Controleert of een tegel bezet is door een fysiek item (bal, etc.).
	 * @param {number} x - Tegel X coördinaat.
	 * @param {number} y - Tegel Y coördinaat.
	 * @returns {boolean} True als de tegel een item bevat (niet zijnde het item dat we slepen).
	 */
	...
	const buildableObjects = [
		...
		...Array(6)
			.fill(null)
			.map((_, i) => ({ name: `Test ${i + 1}`, height: 1, image: "", category: "objecten", subCategory: "blok", placement: "floor" })),
		{ name: "Muurdecoratie", image: wallItemImg.src, category: "objecten", subCategory: "muur", placement: "wall" },
		...Array(8)
			.fill(null)
			.map((_, i) => ({ name: `Test Muur ${i + 1}`, image: "", category: "objecten", subCategory: "muur", placement: "wall" })),
	];

	/**
	 * Rendert de lijst met beschikbare bouw-items in het bouwmenu.
	 * Filtert op basis van de geselecteerde categorie en subcategorie.
	 */
	function renderBuildItems() {
		...
		const itemsToShow = buildableObjects.filter(
			(item) => item.category === activeBuildCategory && item.subCategory === activeObjectSubCategory
		);
	...
	/**
	 * Genereert en toont de knoppen voor subcategorieën (bv. Blok, Muur) in het bouwmenu.
	 */
	function renderObjectSubCategories() {
		...
		const subCategories = [...new Set(buildableObjects.filter((item) => item.category === "objecten").map((item) => item.subCategory))];
	...
	/**
	 * Stelt de actieve bouwtool in.
	 * @param {string} tool - De tool: 'place', 'move' of 'delete'.
	 */
	...
	/**
	 * Update de globaal geselecteerde kleur en de UI-preview.
	 * @param {Object} color - Het iro.js kleurobject of een gelijkwaardig object.
	 * @param {string} source - De bron van de update ('picker' of 'input').
	 */
	...
	/**
	 * Voegt een chatbericht toe aan de lijst en toont het op het scherm.
	 * @param {string} text - De inhoud van het bericht.
	 * @param {string} user - De naam van de afzender (default "Jij").
	 */
	...
	/**
	 * Herbouwt de visuele chatgeschiedenis (het logvenster) op basis van alle berichten.
	 */
	...
	// Hulp functie: heuristiek (Manhattan afstand) voor A*
	/**
	 * Berekent de Manhattan afstand tussen twee punten.
	 * Dit dient als heuristiek voor het A* pathfinding algoritme.
	 */
	 ...
	 * Berekent het kortste pad van start naar eind, rekening houdend met obstakels.
	 *
	 * @param {Object} start - Start coördinaat {x, y}.
	 * @param {Object} end - Eind coördinaat {x, y}.
	 * @returns {Array} Een lijst van coördinaten die het pad vormen, of een lege array als er geen pad is.
	 */
	...
	// Tile naar scherm
	/**
	 * Converteert logische rooster-coördinaten (isometrisch) naar 2D scherm pixels.
	 * @param {number} ix - Isometrische X (grid kolom).
	 * @param {number} iy - Isometrische Y (grid rij).
	 * @returns {Object} Schermcoördinaten {sx, sy}.
	 */
	...
	/**
	 * Converteert muis/ scherm locaties naar rooster-coördinaten.
	 * Wordt gebruikt voor klik-detectie.
	 * @param {number} mx - Muis X.
	 * @param {number} my - Muis Y.
	 * @returns {Object} Grid coördinaten {x, y} (afgerond).
	 */
	...
	/**
	 * Converteert schermcoördinaten naar 'wereld' coördinaten (isometrisch float).
	 * Dit is preciezer dan toTile en wordt gebruikt voor het slepen van items.
	 * @param {number} mx - Muis X.
	 * @param {number} my - Muis Y.
	 * @returns {Object} Wereld coördinaten {x, y} (float).
	 */
	...
	/**
	 * Breekt tekst op in meerdere regels zodat het binnen een bepaalde breedte past.
	 * @param {string} text - De te verwerken tekst.
	 * @param {number} maxWidth - De maximale breedte in pixels.
	 * @returns {Array} Een lijst van strings, waarbij elke string een regel is.
	 */
	...
	/**
	 * Tekent het vloer-raster van de wereld.
	 * Dit omvat de basis tegels, custom gekleurde tegels, en highlights (pad, hover).
	 */
	function drawTiles() {
	...
	/**
	 * Tekent de speler (bal met hoed) op de huidige positie.
	 * De positie wordt berekend inclusief een 'hop' effect tijdens het lopen.
	 */
	function drawBall() {
	...
	// Achterwand
	/**
	 * Tekent de achterste muur (rechtsboven).
	 * Dit wordt gedaan per segment (kolom) met ondersteuning voor kleuren en muur-objecten.
	 */
		...
		for (let x = mapW - 1; x >= 0; x--) {
			...
			if (hoverTarget?.preview && hoverTarget.id === wallId) {
				...
				if (isBuildObjectFlipped) {
					...
					ctx.scale(-1, 1);
					ctx.drawImage(
						hoverTarget.preview.image,
						-objX - hoverTarget.preview.image.width / 2,
						objY - hoverTarget.preview.image.height + 16
					); // 16px omlaag
					ctx.restore();
				} else {
					ctx.drawImage(
	...
	// Linkermuur
	/**
	 * Tekent de linker muur.
	 * Dit wordt gedaan per segment (rij) met ondersteuning voor kleuren en muur-objecten.
	 */
	...
	/**
	 * Telt hoeveel chatballonnen er momenteel actief (zichtbaar) zijn.
	 * @returns {number} Aantal actieve berichten.
	 */
	function getActiveBalloonCount() {
		const now = Date.now();
		return chatMessages.filter((m) => now - m.time < m.duration).length;
	}
	...
	window.addEventListener("keydown", (e) => {
		...
		if (active !== chatInput && !["Shift", "Control", "Alt", "Meta", "Enter"].includes(e.key)) {
			chatInput.focus();
		}
	...
	/**
	 * Tekent de chatballonnen boven het hoofd van de speler.
	 * Beheert de stapeling, typing-indicator en fade-out effecten.
	 */
	function drawChatBallon() {
		...
		// ─────────────────────────────────────────────
		const activeMessages = chatMessages.filter((m) => now - m.time < m.duration);
		...
		for (let i = stack.length - 1; i >= 0; i--) {
			const lines = wrapText(stack[i].text, maxWidth - paddingX * 2);
		...
		stack.forEach((m, i) => {
			...
			if (!m.isTyping) {
				...
				if (age > m.duration - fadeTime) {
					alpha = 1 - (age - (m.duration - fadeTime)) / fadeTime;
				}
			} else {
	...
	/**
	 * Controleert of een punt (x,y) binnen een polygoon ligt.
	 * Gebruikt het Ray-Casting algoritme.
	 * @param {number} x - Test punt X.
	 * @param {number} y - Test punt Y.
	 * @param {Array} vs - Lijst van vertices [{sx, sy}, ...].
	 * @returns {boolean} True als punt binnen polygoon ligt.
	 */
	...
	canvas.addEventListener("mouseup", (e) => {
		...
		if (isItemDragging && e.button === 0) {
			...
			if (isOverInventory && inventory.style.display === "flex") {
				// Alleen toevoegen aan inventaris als deze open is
		...
		if (e.button === 0 && isBuildMode && Math.sqrt(dx * dx + dy * dy) < 5) {
			...
			if (activeBuildCategory === "objecten" && ((x >= 0 && y >= 0) || hoverTarget?.type === "wall")) {
				...
				switch (buildTool) {
					case "place":
						if (
							selectedBuildObject?.placement === "floor" &&
							!isOccupied &&
							!playerOnTile &&
							!isItemTileOccupied &&
							selectedBuildObject.name
						) {
	...
	/**
	 * Update de camera-positie voor soepele bewegingen (panning & zoom).
	 * @param {number} delta - Tijd-delta vermenigvuldiger (1.0 bij 60fps).
	 */
	...
	/**
	 * Update de fysica van losse items (bal, etc.).
	 * Past snelheid, frictie en botsing met muren toe.
	 * @param {number} delta - Tijd-delta vermenigvuldiger.
	 */
	...
	/**
	 * Update de positie van de speler tijdens het lopen (springen tussen tegels).
	 * Handelt ook de interactie (duwen) met items af.
	 * @param {number} delta - Tijd-delta vermenigvuldiger.
	 */
	function updateBall(delta) {
		...
		items.forEach((item) => {
			...
			if (distance < minDistance) {
				const pushForce = 0.05 / item.mass; // Kracht van de duw, beïnvloed door massa
	...
	setInterval(() => {
		const active = chatMessages.filter((m) => Date.now() - m.time < m.duration).sort((a, b) => a.time - b.time);

		if (active.length >= 5) {
			const oldest = active[0];
			const remaining = Math.max(0, Math.ceil((oldest.duration - (Date.now() - oldest.time)) / 1000));
	...
	/**
	 * De hoofd game-loop.
	 * Wordt elke frame aangeroepen door requestAnimationFrame.
	 * Verantwoordelijk voor updaten van logica en het tekenen van alles.
	 * @param {number} time - Huidige tijdstempel.
	 */
	function loop(time) {
		...
		ctx.setTransform(scale, 0, 0, scale, canvas.width / 2 + camX, canvas.height / 4 + camY);
		...
		items.forEach((item) => {
			...
			renderList.push({
				type: "item",
				x: Math.floor(item.x), // Gebruik de tegelcoördinaat voor sortering
				y: Math.floor(item.y), // Gebruik de tegelcoördinaat voor sortering
				draw: () => {
